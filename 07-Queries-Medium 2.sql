...
ALTER TABLE flights 
ADD airline_IATA CHAR(3),
    gate_ID INT;

ALTER TABLE flights 
ADD CONSTRAINT FK_flights_Airline FOREIGN KEY (airline_IATA) REFERENCES Airline(airline_IATA),
    CONSTRAINT FK_flights_Gate FOREIGN KEY (gate_ID) REFERENCES Gate(gate_ID);
...
-- At least 4 airlines with different countries and IATA codes
INSERT INTO Airline VALUES
('EK', 'Emirates', 'United Arab Emirates', 'ops@emirates.com'),
('QA', 'Qatar Airways', 'Qatar', 'support@qatarairways.com'),
('BA', 'British Airways', 'United Kingdom', 'info@ba.com'),
('LH', 'Lufthansa', 'Germany', 'operations@lufthansa.de');

-- At least 8 gates distributed across at least 3 airports. Each gate must belong to an airport that already exists in your database
INSERT INTO Gate (gate_code, terminal_name, IATA) VALUES
('A1', 'Terminal 3', 'DXB'), 
('A2', 'Terminal 3', 'DXB'), 
('B1', 'Terminal 1', 'LHR'), 
('B2', 'Terminal 1', 'LHR'), 
...
UPDATE flights SET airline_IATA = 'QA', gate_ID = 7 WHERE flights_ID = 6;
UPDATE flights SET airline_IATA = 'BA', gate_ID = 4 WHERE flights_ID = 7;
UPDATE flights SET airline_IATA = 'EK', gate_ID = 2 WHERE flights_ID = 8;

--At least 10 baggage records distributed across different bookings. Include both Cabin and Checked types. Vary the weights

INSERT INTO Baggage1 (tag_number, weight_kg, type, booking_id) VALUES 
('TGN-1001', 7.50, 'Cabin', 1),   
('TGN-1002', 23.00, 'Checked', 1), 
('TGN-1003', 18.25, 'Checked', 2), 
('TGN-1004', 5.50, 'Cabin', 3),    
('TGN-1005', 30.00, 'Checked', 4),
('TGN-1006', 8.10, 'Cabin', 5),    
('TGN-1007', 22.40, 'Checked', 6), 
('TGN-1008', 6.00, 'Cabin', 7),    
('TGN-1009', 25.50, 'Checked', 8), 
('TGN-1010', 7.20, 'Cabin', 9),    
('TGN-1011', 19.80, 'Checked', 10);
select * from booking
-- At least 4 delay log records. Link them only to flights that have a status of 'Delayed' or 'Cancelled' in your existing data
INSERT INTO FlightDelayLog (reason, duration_minutes, flight_id) VALUES 
('Heavy fog at LHR causing runway congestion', 45, 2),
('Delayed catering delivery and cabin preparation', 20, 2),
('Severe weather warning: Typhoon at destination airport', 480, 3),
('Technical fault in the aircraft braking system', 120, 3);
...
-- Basic Level 2:
-- List all airlines and their country of registration, ordered alphabetically by airline name
SELECT 
    airline_name, 
    country 
FROM Airline
ORDER BY airline_name ASC;
-- Show all gates and the airport they belong to
SELECT 
    g.gate_code, 
    g.terminal_name, 
    a.names AS airport_name, 
    a.city, 
    a.country
FROM Gate g
JOIN airports a ON g.IATA = a.IATA
ORDER BY a.names, g.gate_code;
-- List all baggage records and their type, ordered by weight from heaviest to lightest
SELECT 
    tag_number, 
    type, 
    weight_kg 
...
-- Show all delay log records and the flight they belong to, ordered by recorded datetime
SELECT 
    f.flight_number, 
    dl.reason, 
    dl.duration_minutes, 
    dl.recorded_at 
FROM FlightDelayLog dl
JOIN flights f ON dl.flight_id = f.flights_ID
ORDER BY dl.recorded_at ASC;
-- List all flights that currently have no gate assigned
SELECT 
    flight_number, 
    status1 
FROM flights 
...
--Medium Level 2:
-- Show each flight with its flight number, airline name, and gate code.
SELECT 
    f.flight_number, 
    a.airline_name, 
    g.gate_code
...
-- List all baggage items along with the passenger name and flight number they are linked to
SELECT 
    bg.tag_number, 
    p.names AS passenger_name, 
    f.flight_number
FROM Baggage1 bg
JOIN booking b ON bg.booking_id = b.booking_ID
JOIN Passenger p ON b.national_ID = p.national_ID
JOIN flights f ON b.flights_ID = f.flights_ID;
--Count the total number of baggage items per booking. Show the booking ID, passenger name, and baggage count
SELECT 
    b.booking_ID, 
    p.names AS passenger_name, 
    COUNT(bg.baggage_id) AS total_baggage_items
FROM booking b
JOIN Passenger p ON b.national_ID = p.national_ID
LEFT JOIN Baggage1 bg ON b.booking_ID = bg.booking_id
GROUP BY b.booking_ID, p.names;
-- Show all delay logs including the flight number, airline name, delay reason, and duration in minutes
SELECT 
    f.flight_number, 
    a.airline_name, 
    dl.reason, 
...
JOIN flights f ON dl.flight_id = f.flights_ID
JOIN Airline a ON f.airline_IATA = a.airline_IATA;
-- Find the total weight of checked baggage per flight. Show the flight number and total checked weight
SELECT 
    f.flight_number, 
    SUM(bg.weight_kg) AS total_checked_weight
FROM flights f
JOIN booking b ON f.flights_ID = b.flights_ID
JOIN Baggage1 bg ON b.booking_ID = bg.booking_id
WHERE bg.type = 'Checked'
GROUP BY f.flight_number;
-- Count how many flights each airline operates. Order from highest to lowest
SELECT 
    a.airline_name, 
    COUNT(f.flights_ID) AS flight_count
FROM Airline a
LEFT JOIN flights f ON a.airline_IATA = f.airline_IATA
GROUP BY a.airline_name
ORDER BY flight_count DESC;
-- Show all flights that have been delayed more than once. Display the flight number and total number of delay records
SELECT 
    f.flight_number, 
    COUNT(dl.delay_id) AS total_delay_records
FROM flights f
JOIN FlightDelayLog dl ON f.flights_ID = dl.flight_id
GROUP BY f.flight_number
HAVING COUNT(dl.delay_id) > 1;
